...
public class AdministracaoController {
	...
	public String atualizaProfessores(Model model) {
		...
		List<Usuario> usuarios = usuarioService
				.getByAffiliation(Constants.AFFILIATION_DOCENTE);
	...
	private void atualizaVagas() {
		Periodo periodoAtual = periodoService
				.getPeriodoPosterior(periodoService
						.getPeriodoPosterior(periodoService.getPeriodoAtual()));
		List<Periodo> periodos = periodoService
				.getPeriodosPosteriores(periodoAtual);
	...
	public String getReservas(Model model) {
		...
		if (periodo != null) {
			...
			if (periodo != null) {
				...
				ranking.setTuplas(rankingService.visualizarRanking(
						periodo.getAno(), periodo.getSemestre()));
	...
	@CacheEvict(value = { "default", "reservasByProfessor", "periodo",
			"visualizarRanking", "ranking", "loadProfessor", "professores" }, allEntries = true, beforeInvocation = true)
	public String atualizarRanking(HttpServletRequest request,
			RedirectAttributes redirect) {
		String[] status = request.getParameterValues("status");
		for (String s : status) {
			String[] valor = s.split("-");
			Reserva reserva = reservaService.find(Reserva.class,
		...
		for (TuplaRanking tupla : ranking.getTuplas()) {
			if (tupla.getReserva().getAnoInicio().equals(ano)
				...
				Afastamento afastamento = afastamentoService.getByReserva(tupla
						.getReserva());
		...
		redirect.addFlashAttribute(Constants.INFO,
				Constants.MSG_RESERVAS_ATUALIZADAS);
	...
	@CacheEvict(value = { "default", "reservasByProfessor", "periodo",
			"visualizarRanking", "ranking", "loadProfessor", "professores" }, allEntries = true)
	public String atualizaAdmissao(@RequestParam("id") Long id,
		...
		model.addAttribute("info", "Data de admissão do(a) Prof(a) "
				+ professor.getNome() + " atualizada com sucesso.");

		return Constants.PAGINA_LISTAR_PROFESSORES;
	}

	@RequestMapping(value = "/edit-periodo.json", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody Model permissaoEditPeriodo(Model model,
	...
	@CacheEvict(value = { "default", "reservasByProfessor", "periodo",
			"visualizarRanking", "ranking", "loadProfessor", "professores" }, allEntries = true, beforeInvocation = true)
	public Model editarPeriodo(Model model, @RequestParam("id") Long id,
			...
			if (periodo.getStatus().equals(StatusPeriodo.ABERTO)) {
				...
				try {
					...
					if (encerramento.before(today)) {
						model.addAttribute(Constants.ERRO,
								Constants.MSG_DATA_FUTURA);
					...
					model.addAttribute(Constants.ERRO,
							Constants.MSG_ERRO_ATUALIZAR_PERIODO);
					return model;
				}
			}
		}
