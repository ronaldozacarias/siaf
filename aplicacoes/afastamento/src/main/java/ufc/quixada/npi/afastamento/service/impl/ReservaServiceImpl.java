...
public class ReservaServiceImpl extends GenericServiceImpl<Reserva> implements ReservaService {
	...
	public List<Reserva> getReservasByPeriodo(Integer ano, Integer semestre) {
		...
		return reservaRepository
				.find(QueryType.JPQL,
						"from Reserva where :ano >= anoInicio and :ano <= anoTermino and id not in (select id from Reserva where "
								+ "(anoInicio = :ano and semestreInicio > :semestre) or (anoTermino = :ano and semestreTermino < :semestre))",
						params);
	}
	...
	public List<Reserva> getReservasAnterioresComPunicao(Professor professor, Periodo periodo) {
		...
		return reservaRepository
				.find(QueryType.JPQL,
						"from Reserva where status = 'CANCELADO_COM_PUNICAO' and professor.cpf = :cpf and (anoTermino < :ano or (anoTermino = :ano and semestreTermino < :semestre))",
						params);
	}
