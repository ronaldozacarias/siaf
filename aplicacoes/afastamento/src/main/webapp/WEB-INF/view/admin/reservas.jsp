	...
	<div id="wrapper">
		...
		<div id="content">
			...
			<c:if test="${not empty ranking}">
				<form id="atualizarRanking"
					action="/siaf/administracao/atualizar-ranking" method="POST">
					<input type="hidden" value="${ranking.periodo.ano }" name="ano" />
					<input type="hidden" value="${ranking.periodo.semestre }"
						name="semestre" />
					<div id="periodo">
					...
					<table id="ranking" class="table">
						...
						<tbody>
							<c:forEach items="${ranking.tuplas }" var="tupla"
								varStatus="count">
								<tr class="${tupla.status }">
									...
									<td class="align-center"><c:choose>
											<c:when
												...
												<select id="${tupla.reserva.id }" name="status"
													class="form-control selectpicker">

													<option ${tupla.status == 'CLASSIFICADO' ? 'selected' : ''}
														value="${tupla.reserva.id }-ABERTO">CLASSIFICADO</option>
													<option ${tupla.status == 'ACEITO' ? 'selected' : ''}
														value="${tupla.reserva.id }-ACEITO">ACEITO</option>
													<option ${tupla.status == 'CANCELADO' ? 'selected' : ''}
														value="${tupla.reserva.id }-CANCELADO">CANCELADO</option>
													<option
														${tupla.status == 'CANCELADO_COM_PUNICAO' ? 'selected' : ''}
														value="${tupla.reserva.id }-CANCELADO_COM_PUNICAO">CANCELADO
														COM PUNIÇÃO</option>
												</select>
											</c:when>
											<c:otherwise>
					...
					<c:if test="${atualizar }">
						<div class="controls">
							<input name="reservar" type="submit" class="btn btn-siaf"
								value="Atualizar Ranking" />
						</div>
					</c:if>
				</form>
			</c:if>
